...
assert.throws(() => wasm.replace({ title: 'no id' }))
...
assert.throws(() => wasmFloatId.add({ id: 1, text: 'duplicate' }))
...
assert.throws(() => MiniSearchWasm.loadMiniSearchJSON(JSON.stringify({ ...js.toJSON(), serializationVersion: 3 }), options))
assert.throws(() => MiniSearchWasm.loadMiniSearchJSON(JSON.stringify(js), { fields: ['other'] }))
assert.throws(() => MiniSearchWasm.loadMiniSearchJSON('{"documentCount": 1}', options))
